...
def main(req: func.HttpRequest, creditcard: func.SqlRowList) -> func.HttpResponse:
    # Convert SqlRowList to list of dictionaries
    rows = list(map(lambda r: json.loads(r.to_json()), creditcard))
    ...
    try:
        ...
        # Check if any row matches the given 'id', if so, return failure response
        if any(row.get('id') == id for row in rows):
            return func.HttpResponse("Failed to delete credit card.", status_code=500)
        else:
            # Return success response if 'id' doesn't match any row
            return func.HttpResponse("Credit Card deleted successfully", status_code=200)
